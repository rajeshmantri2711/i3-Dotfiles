#!/bin/zsh
# Set the theme
ZSH_THEME="kali-like-zsh-theme"

# Enable syntax highlighting
ZSH_SYNTAX_HIGHLIGHTING_PATH="${HOME}/.zsh/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh"
if [[ -f "${ZSH_SYNTAX_HIGHLIGHTING_PATH}" ]]; then
    source "${ZSH_SYNTAX_HIGHLIGHTING_PATH}"
else
    echo "Syntax highlighting not found. Please install it."
fi
...
if [[ -f "${ZSH_AUTOSUGGESTIONS_PATH}" ]]; then
    ...
    echo "Auto-suggestions not found. Please install it."
fi
...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'$CYANPROMPT┌───\(%B$FGPROMPT%n@%m%b$CYANPROMPT)-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b$CYANPROMPT]$()\n└─%B%(#.%F{red}#.$FGPROMPT$)%b%F{reset} '
            RPROMPT=
            ;;
        oneline)
             PROMPT=$'%F{CYANPROMPT}%~%F{reset}> '
             RPROMPT=
            ;;
    esac
}
